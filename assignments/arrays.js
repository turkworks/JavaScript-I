...
console.log(
  `The last car in the inventory is a ${lastCar["car_year"]} ${
    lastCar["car_make"]
  } ${lastCar["car_model"]}`
);
...
for (year in carYears) {
  if (carYears[year] < 2000) {
    oldCars.push(carYears[year]);
  }
}
console.log(oldCars);
...
for (cars in inventory) {
  if (
    inventory[cars]["car_make"] === "Audi" ||
    inventory[cars]["car_make"] === "BMW"
  ) {
    BMWAndAudi.push(inventory[cars]["car_make"]);
  }
}
